...
async def raid(e):
    if e.sender_id in SUDO_USERS:
        ...
        try:
            if uid in ALTRON:
                await e.reply("ɴᴏ, ᴛʜɪꜱ ɢᴜʏ ɪꜱ ⚡🇩𝜩Ⓐ︎🇩 ⚡ ᴏᴡɴᴇʀ.")
            elif uid == OWNER_ID:
                await e.reply("ɴᴏ, ᴛʜɪꜱ ɢᴜʏ ɪꜱ ᴏᴡɴᴇʀ ᴏꜰ ᴅᴇᴀᴅ sᴘᴀᴍ.")
            elif uid in SUDO_USERS:
                await e.reply("ɴᴏ, ᴛʜɪꜱ ɢᴜʏ ɪꜱ ᴀ ꜱᴜᴅᴏ ᴜꜱᴇʀ ᴏғ ᴅᴇᴀᴅ sᴘᴀᴍ.")
            else:
    ...
    if e.sender_id in SUDO_USERS:
        ...
        try:
            ...
            if user_id in ALTRON:
                ...
                await e.reply("ɴᴏ, ᴛʜɪꜱ ɢᴜʏ ɪꜱ ᴀ ꜱᴜᴅᴏ ᴜꜱᴇʀ ᴏꜰ ᴅᴇᴀᴅ sᴘᴀᴍ.")
            else:
                global REPLY_RAID
                check = f"{user_id}_{e.chat_id}"
                if check not in REPLY_RAID:
                    REPLY_RAID.append(check)
                await e.reply("» ʙᴏʟ ᴍᴄ ᴀʙ ᴛᴜ !! ✅")
        except NameError:
...
async def drraid(e):
    if e.sender_id in SUDO_USERS:
        ...
        try:
            ...
            await e.reply("» ᴊᴀ ʀᴀɴᴅɪ ᴍᴀғ ᴋɪʏᴀ !! ✅")
        except NameError:
    ...
    if e.sender_id in SUDO_USERS:
        ...
        try:
            if uid in ALTRON:
                ...
                await e.reply("ɴᴏ, ᴛʜɪꜱ ɢᴜʏ ɪꜱ ᴀ sᴜᴅᴏ ᴜsᴇʀ ᴏꜰ ᴅᴇᴀᴅ sᴘᴀᴍ.")
            else:
                first_name = entity.first_name
                counter = int(xraid[1])
                username = f"[{first_name}](tg://user?id={uid})"
